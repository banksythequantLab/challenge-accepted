...
| **BROKEN right now** | **FORGE is building 1 of 6, not 7 of 7.** Four live runs since revision 00038 measured 6/6 (orphaned by a duplicate-charter bug, since fixed), 1/6, 3/7 and 1/6. `CA_FORGE_DEBUG=1` says what happens: all four workers start with four distinct real specs, all four reach the model with the spec in the prompt, and then exactly one makes a second call while the other three vanish — with `after_agent_callback` firing for none of them, so they are torn down rather than idle. Ruled out by experiment: embeddings (same result with `CA_EMBED=off`) and prompt size (same result at 1,833 chars as at 4,548). The row below is what this used to do and will say so until a live run proves it again |
