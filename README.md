...
| Verified live | **A tool that throws when you open it is no longer stored as "passed".** The *10k Race Day Pacing Strategy Calculator* shipped with `smoke_test_passed=True` and threw `Cannot set properties of null` the moment the dashboard opened it — one line reading `getElementById('h2-mi')` against markup that declares `h1-mi`, `h2-final`, `h2-pace` and `h2-tot` but no `h2-mi`. Twenty-one ids declared, twenty-one looked up, one that does not line up, and every line after it stops running. The Toolwright could not have caught it: its smoke test runs the *calculation*, in a code executor with no DOM, so it tests the half it can reach and certifies the half it cannot. `save_tool` now checks it in code — the tool is still saved, but marked degraded, with the missing ids named so a re-save is a one-line fix. The rule is deliberately timid (an id is only reported when its literal appears in the source exactly once) because marking a *working* tool degraded is the worse failure. **Measured over every tool this product has ever built: 425 tools, 168 using `getElementById`, exactly 1 flagged — the one that actually threw in a real browser.** Nine unit tests hold the false-positive line: ids assigned to created elements, injected via `innerHTML`, or built from template literals all survive |
| Verified | **265** tests pass against a real ADK `Runner`, plus **34** checks in `scripts\check_*` that drive the actual controls; FastAPI boots, `/api/healthz` 200. All of them re-run against the current build after today's deploys -- not carried over from a green run last week. That count was stale at *137 tests, 18 live checks*, and then again at *235*, which is a small lie of the same species as the auth row below -- and a row that brags about catching its own stale number going stale a second time is the joke writing itself. It is checked now with `dir /b scripts\check_*.py | find /c /v ""` rather than remembered |
...
| Verified live *(four consecutive runs, and the batch size was the wrong lead)* | **FORGE builds every tool it specs — 5/5, 7/7, 6/6, 6/6 back to back, in ~5 minutes.** Those four runs were measured at four concurrent workers; **the setting is now eight**, and the run that settled it was **8 specs asked / 8 tools built** on a clean `forge_workers.toolwright_0` branch. Halving the batch to two made it pass three times running, so this row used to say concurrency was the problem. **It was not.** A trace added afterwards showed FORGE running inside the Cartographer's tool frame on every failure and never on a success; the real fix is `disallow_transfer_to_parent=True` on the single-turn sub-agents, plus an Interviewer prompt that calls `finish_task` instead of signing off to the user — both in Known issues. Two workers narrowed the window the bug needed and never closed it. With the causes fixed the batch went back up and the demo gets ~3 minutes back. The lesson is worth more than the number, and `config.FORGE_WORKERS` keeps it: **a change that makes a symptom go away three times running is still not a diagnosis.** What follows is the evidence that led to the wrong answer, kept because the reasoning was sound and the conclusion was not. It had been building 1 of 6. Four live runs measured 6/6 (orphaned by a duplicate-charter bug, since fixed), 1/6, 3/7, 1/6, and `CA_FORGE_DEBUG=1` said what happened: all four workers start with four distinct real specs, all four reach the model with the spec in the prompt, then exactly one makes a second call while the other three vanish — with `after_agent_callback` firing for none of them, so they were torn down rather than idle. That is the signature of an exception inside the `asyncio.TaskGroup` ParallelAgent runs them in, where one failure cancels its siblings. Embeddings (`CA_EMBED=off`) and prompt size (1,833 chars vs 4,548) were each ruled out by experiment first; neither changed anything. Halving the batch did: **7 asked / 7 built, twice in a row** on revision 00050, every worker reaching END, the queue draining and the dispatcher escalating cleanly. Then **8 asked / 8 built on revision 00051** with `CA_FORGE_WORKERS` unset — the same result from the committed default rather than from an env var somebody has to remember to set. **The cause is still unidentified** — no `RESOURCE_EXHAUSTED`, no 429, and the ERROR logs carry only OpenTelemetry context-teardown noise, which is a symptom of the cancellation rather than its cause. So this is a floor that works, not a diagnosis, and `config.FORGE_WORKERS` says so. Cost: four batches instead of two, ~8.5 min instead of ~5. Coverage is unaffected — the loop reseeds until the queue drains. **That paragraph was written before the cause was found, and every sentence in it is true and beside the point.** |
...
| Verified live | **A party can keep one record — and a save that would overwrite a teammate's is refused with theirs attached.** Tool state was per-person, full stop, which is right for a training log and wrong for a cost split: two people coordinating through the same tool each saw their own empty copy, a failure indistinguishable from "nobody has filled it in yet". Scope is now decided at build time by the Quartermaster (`ToolSpec.shared`) and read off the stored tool, never off a query parameter the caller could pick. Shared writes carry the version they read: `scripts\check_shared_tool_live.py` drives two signed-in users through it — Dana saves, Derek sees her numbers and her name; her personal log stays invisible to him on the same challenge at the same moment; his stale save is **409 with the winning state attached**, so his client can say *who* changed it; he reloads, redoes it, it lands; a stranger is 403 throughout. `scripts\check_shared_tool_ui.py` drives the same story in a real browser and **found a bug reading the code would not have**: the explanation was being wiped by the re-render it triggered, so the user watched their number become somebody else's with nothing on screen saying why. 14 unit tests. The version check is read-then-write rather than a transaction, and `store.put_tool_state` says so rather than implying an atomicity it does not have. Both checks pass on **revision 00066** at `challengeaccepted.app` (last re-run after the read-cost and Gemma changes, not carried over from 00051), and each seeds its own challenge and deletes it rather than leaving test quests on somebody's dashboard |
| Verified live | **An open model does real work here, and it calls tools rather than describing them.** The Archivist runs on **Gemma 4** (`gemma-4-26b-a4b-it-maas`) — the third rung of a ladder that already went reasoning tier -> Flash-Lite: it only transcribes, and its output is additive, so a weaker answer costs a thinner journal entry rather than a broken phase. It rides the *same* Vertex credentials and global endpoint as everything else — no API key, no deployed endpoint, no GPU — which is the whole reason it was affordable to do, since a second auth surface is what caused the worst outage in this repo. The risk was never quality, it was **prose**: an agent whose job is to *call* `write_journal` and `remember_group_fact` but which instead replies *"Sure, I've noted that down"* looks healthy, logs nothing, and takes the party's shared memory with it. So that was measured first, against the live endpoint with the real tool schemas — **2 tool calls, no prose, 2.36s** against Flash-Lite's 1.79s. Then `check_party_live.py` was re-run on production with Gemma serving: a teammate inherited *"Must host on Vercel due to GCP billing/admin restrictions"* and was handed an open node. `scripts\check_archivist_model.py` asks the prose question on demand; `/api/healthz` names the serving model per tier, so this is checkable from outside. Backing it out is one `gcloud run services update`, no rebuild |
| Verified | **The idle dashboard got 29% cheaper to leave open.** `scripts\check_poll_cost.py` sat at exactly 120 Firestore reads/minute against a budget of 120 — passing, no headroom, *per open browser*, and at that rate one page left open exhausts the 50k/day free read quota in about seven hours. Two of the eight reads per poll were waste and neither was data: `/dashboard` read the group doc for shared facts and then `_party` read the same doc again, and every member's profile was re-read on every poll to turn a uid into a name. Now 85/min, and the per-member part no longer scales with how long you watch. The roster is still read fresh every poll — *"a teammate joins and the header goes 1 -> 2 while you sit still"* is the demo, and a cache that held it for a minute would break exactly that. The check now asserts the **composition** of a poll rather than an average, because an average tight enough to catch a one-read regression also flaps; proved by re-breaking both fixes on purpose, where the old 120/min ceiling passed them at 100 and 105 |
...
| Verified live | **A joining teammate is handed live work, not finished work.** This row said *Not verified* for weeks, then *Verified* on the strength of `scripts\check_handoff.py`, which spins up its own local server against a seeded challenge — honest, but local, and this project has already been burned once by exactly that gap. It is now proved on the deployed service by `check_climb_live.py`, which is standing in the best possible place to do it: having just closed a node on production with real evidence, in a session still open, it asks *"what should I pick up next?"* and checks which node the Coach names **first**. Deliberately the same session rather than a fresh one — a new session forces a state reload and makes the claim trivially true. Mentioning a done node is fine and often right; leading with one is the failure, because it says plainly that the agent did not read the state it claims to read. Live result: closed `establish-training-schedule` on evidence, and the Coach led with *"Select fitted running shoes and equipment"* — open work. `check_handoff.py` still covers the cold-join case locally, where Dana gets both party facts (one attributed to Derek by name) and is pointed at an open node |
...
### Fixed: one run in two stopped dead after the charter was saved -- the prompt said to

Not the nesting bug after all, and not a framework quirk either: **the Interviewer's
prompt instructed it to do the exact thing that stalls the run.** Its last line read
*"call `save_charter`, tell the user in one sentence what you heard, and finish"*.

`mode="task"` agents run until they call `finish_task`. ADK is explicit: *"If the agent
outputs text directed to the user instead of calling `finish_task`, the framework pauses
execution and delivers the message"*, and *"a task will not complete successfully if the
task agent fails to call `finish_task`"*. So "tell the user … and finish" is an
instruction to send a message and stop -- which the framework reads as a question to the
user, and waits forever for an answer nobody is coming to give.

The trace caught it once tool calls were logged as they happened rather than only in
the end-of-turn summary (the summary fires on the root agent's exit, and the failures
worth diagnosing are exactly the ones where the root never exits):

```
22:25:23  [FLOW] enter interviewer  branch=interviewer@call_2058
22:25:35  [FLOW] leave interviewer
          (nothing. no warden, no cartographer, no forge, no error)
```

A healthy run has `enter warden branch=-` on the very next line.

The prompt now names `finish_task`, says it must happen in the same turn as
`save_charter`, and says why. On revision 00057 the trace reads
`[FLOW] tool finish_task (interviewer)` and the run built **6 of 6**.
`tests/test_interviewer_finishes.py` pins the wording -- this is a load-bearing
sentence sitting in a file full of prose nobody diffs carefully.

Worth naming the pattern: three separate failures in this project have now been a
correct-looking instruction or setting that quietly meant "do not continue". None of
...
the dispatcher escalating cleanly out of the loop. The second reached `save_charter`
and stopped. Final turn: **25 seconds, one tool call, zero nodes, zero specs, zero
tools.** The journal ends at *"Charter locked: Run a 10k race in under 55 minutes on
Christmas Eve"* and nothing follows it.

So the graph was never drawn. FORGE cannot fail to build tools for a graph that does
not exist, and `check_forge_live.py` reported `FAIL: the deployed service built
nothing` — which is true, and points at the wrong agent. **The Warden did not hand off
to the Cartographer.** There are no ERROR logs for the window; the turn simply ended.

`prompts.WARDEN` already carries a rule about exactly this ("do not claim a silent
delegation happened"), which is evidence the failure mode is known and not that it is
handled. Not chased yet. What it needs is a run with the delegation traced the way
`CA_FORGE_DEBUG=1` traces the workers — right now the only signal is an absence, and
an absence is what every other version of this bug has also looked like.

Nothing above claims otherwise: the FORGE rows say what two workers measured, and this
says that one run in two never got far enough to measure.

### Fixed: the event stream can be cut while FORGE is thinking

`check_forge_live.py` died on `http.client.IncompleteRead(313252 bytes read)` in the
middle of a run that was working. FORGE goes up to two minutes between SSE events while
a Toolwright waits on the model, and a stream that quiet occasionally gets cut
somewhere in the path. The server did not care — the logs show every worker reaching
`END` for four more minutes after the client stopped listening, and the challenge ended
with 8 tools.

The check now tells the two apart, because it could not before: a truncated read prints
a loud warning, stops sending further turns (stacking a second invocation on a session
that is still running measures something else entirely), and then waits for the tool
count to stop moving before reporting — reading the store rather than the stream. A
check that cannot distinguish "I stopped watching" from "it stopped working" is worse
than no check, and this one spent a run accusing the product of the first.
...
### Fixed: two thirds of the tools were a file to read, not a tool to use

The pitch is one sentence: *every other AI gives you a plan, this one builds you the
tools.* On a real challenge on the deployed service, measured by
`scripts\check_tool_render.py`:
...
**Option 1, and it works: 7 of 7 on the next live run.** The Python did not go away --
the Toolwright still proves the arithmetic by running it, because that discipline is
what makes the tool correct. It then ports the verified formula into inline JavaScript
and renders the smoke test's own example on load inside a `data-smoke` element. That
element is the whole safety net: a hand port from Python to JavaScript that nothing
checks is a hand port that drifts, and `--browser` reads it back out of a real iframe.

Honest cost: the FORGE turn went from **170s to 361s**. Every worker now does two
pieces of work instead of one. That is the price of the tool being usable, and the
check prints both numbers.

**And then the browser pass found what the predicates could not.** All seven looked
runnable. Opened in a real sandboxed iframe, one threw at the top of its script:
...
A sandboxed iframe has no storage, and *reading* `localStorage` throws. The Aerobic
Base Phase Running Log was a white rectangle to anyone who opened it, and it passed
everything that did not actually run it. Two fixes, because they answer different
questions: the Toolwright is told never to touch browser storage (and to offer a Copy
button rather than a save button that silently loses somebody's week), and `app.html`
injects an in-memory stub ahead of every tool page — which is the half that covers the
tools built *before* the prompt changed, of which there are forty-five challenges'
worth. Loosening the sandbox was never on the table: `allow-same-origin` would hand
model-written JavaScript the dashboard's own origin, session and token.

The check was wrong first, and that is worth recording too. It faked the sandbox with
`page.set_content` plus `add_init_script` — and `add_init_script` does not apply to
`set_content`, it only runs on navigation, so the shim never executed and a tool that
works in the product was reported as broken. Simulating an environment is how a check
ends up measuring its own simulation. It now loads each tool into a real sandboxed
iframe with the product's actual flags, and reads the product's actual shim out of
`app.html` rather than keeping a second copy that nobody would notice going stale.
...
2. **Group-scoped memory.** Memory Bank is live and proven (see Known issues), but it
   ...
   `remember_group_fact` -> Firestore.

   It is at least **bounded** now: 40 facts reach the prompt and the model is told how
   many did not, verified on production with `check_fact_budget_live.py`. Bounded is
   not the same as scaled. The ranking is content-word overlap -- a lexical guess that
   demonstrably drops an old fact sharing no words with the goal -- and the real fix is
   a second memory scope keyed on `group_id` with embeddings behind it. The obvious
   risk is writing one person's private context into a group everyone can read, so it
   needs a rule about what is shareable before it needs code.
3. ~~**Firebase Auth, replacing the anonymous `localStorage` ids.**~~ **Done, and it
   ...
   challenges are owned outright by test identities, and none of them is sitting on a
   real person's quest. The mess I went looking for was not there, which is worth
   writing down as plainly as if it had been.

   And the checks that drive production had to be taught to sign in one at a time; two
   of them were blind to the deployed service for several revisions, which is precisely
   when a Vertex crash came back unnoticed.
